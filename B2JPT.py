...
def save_connections(ending=''):
    ...
    for connName in save_conns:
        ...
        np.save('weights/' + connName + ending, connListSparse)


def save_theta(ending=''):
    print('save theta')
    for pop_name in population_names:
        np.save('weights/theta_' + pop_name + ending, neuron_groups[pop_name + 'e'].theta)
...
for subgroup_n, name in enumerate(population_names):
    ...
    if test_mode or weight_path[-8:] == 'weights/':
        neuron_groups['e'].theta = np.load(weight_path + 'thetaTest_' + name + ending + '.npy') * b2.volt
    else:
